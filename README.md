...
Copy `vaudio.dll` and `vaudio.xml` from the `2d/dotnet/dev/` folder in the Vercidium Audio SDK, to the `addons/vaudio-godot-mono-openal-2d/bin/` folder.
...
You should then see a `[vaudio-godot-mono-openal-2d] Vercidium Audio (vaudio) plugin enabled` log in Godot's output window.

### 3. Automatic Dependency Setup

The plugin setup script in `addons/vaudio-godot-mono-openal-2d/plugin.gd` will perform some setup logic for you.
