...
public partial class VAWorld
{
    ...
    // sends this editor's own 2D viewport transform to the running game over the debugger protocol,
    // received by OnSyncViewportCamera below. Goes through
    ...
    void SendViewportCameraToRunningGame()
    {
        ...
        // The 2D editor has no queryable Camera2D - its view is the viewport's global canvas
        // transform. Invert it to recover the world-space point at the centre of the view, the
        // view rotation, and the zoom (canvas scale).
        var canvas = viewport.GlobalCanvasTransform;
        var view = canvas.AffineInverse();
    ...
    // "vaudio:sync_viewport_camera" debugger message every editor frame with its own 2D viewport
    // view centre (world space), rotation (radians) and zoom, so long as SOME VAWorld's
